...
$(document).ready(function() {
	...
	$('#submit_it').click(function(e) {
		...
		ajaxObj = {  
			type: "POST",
			//url: "http://54.245.236.121:7001/com.telemedicine.pucit/api/addpatient",
			url: "http://localhost:7001/nosql.telemedicine.pucit/api/addpatient",
	...
	$('#submit_it2').click(function(e) {
		...
		ajaxObj = {  
			type: "POST",
			url: "http://localhost:7001/nosql.telemedicine.pucit/api/add",
			//url: "http://54.245.236.121:7001/com.telemedicine.pucit/api/addperscription",
			data: JSON.stringify(jsObj), 
			contentType:"application/json",
			error: function(jqXHR, textStatus, errorThrown) {
				console.log("Error " + jqXHR.getAllResponseHeaders() + " " + errorThrown);
